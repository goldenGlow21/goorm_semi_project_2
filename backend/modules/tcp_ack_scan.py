...
def ack_scan(target, port):
    ...
    # ACK 패킷 생성
    packet = IP(dst=target) / TCP(sport=get_port(), dport=port, flags="A")  # / ip와 tcp를 묶는 연산자
...
def multi_ack_scan(target, start_port, end_port):
    start = time.time()
    ...
    filtered_port = list(filter(None, results)) # 필터링된 포트
    filtered_port.sort()
    end = time.time()
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("Usage: python tcp_ack_scan.py <target> <start_port> <end_port>")
        sys.exit(1)

    target_ip = sys.argv[1]
    start_port = int(sys.argv[2])
    end_port = int(sys.argv[3])
    ports = multi_ack_scan(target_ip, start_port, end_port)

    if ports:
        print(f"방화벽 설정 됨 : {ports}")
    else:
        print("방화벽 설정 안됨")
